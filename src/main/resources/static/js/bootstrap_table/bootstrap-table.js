...
!function ($) {
    ...
    BootstrapTable.DEFAULTS = {
        ...
        sortSide: 'server', // sort in client or server
    ...
    BootstrapTable.prototype.onSort = function (event) {
        ...
        if(this.options.sortSide != 'client'){
        	if (this.options.sortSide === 'server' || this.options.sidePagination === 'server') {
        		this.initServer(this.options.silentSort);
        		return;
        	}
        }
        //modified by qun.zheng
      /*  if (this.options.sortSide === 'server') {
            this.initServer(this.options.silentSort);
            return;
        }*/
    ...
    BootstrapTable.prototype.initBody = function (fixedScroll) {
        ...
        for (var i = this.pageFrom - 1; i < this.pageTo; i++) {
            ...
            $.each(this.header.fields, function (j, field) {
                ...
                var fieldData = item['_'+field+'_data'];
                if(fieldData && fieldData.hasOwnProperty('visible') && !fieldData.visible){
                	return;
                }
        ...
        this.$body.find('> tr[data-index] > td').off('click dblclick').on('click dblclick', function (e) {
            ...
            that.trigger(e.type === 'click' ? 'click-row' : 'dbl-click-row', item, $tr,e);
    ...
    BootstrapTable.prototype.trigger = function (name) {
        ...
        //parse jquery event obj
        var attempEvent = args[args.length - 1];
        var actualEvent = null;
        if(attempEvent && attempEvent.originalEvent){
        	//it was a jquery event obj;
        	actualEvent = attempEvent;
        ...
        var e1 = $.Event(name);
        if(actualEvent){
        	e1.wrappedEvent = actualEvent;
        }
        this.$el.trigger(e1, args);

        this.options.onAll(name, args);
        var e2 = $.Event('all.bs.table');
        if(actualEvent){
        	e2.wrappedEvent = actualEvent;
        }
        this.$el.trigger(e2, [name, args]);
    };
